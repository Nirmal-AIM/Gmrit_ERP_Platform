...
router.get('/blooms-level', isAuthenticated, isAdmin, async (req, res) => {
    try {
...
router.post('/blooms-level', isAuthenticated, isAdmin, async (req, res) => {
    try {
...
router.put('/blooms-level/:id', isAuthenticated, isAdmin, async (req, res) => {
    try {
...
router.delete('/blooms-level/:id', isAuthenticated, isAdmin, async (req, res) => {
    try {
        const level = await BloomsLevel.findByPk(req.params.id);
        if (!level) return res.status(404).json({ success: false, message: 'Bloom\'s level not found' });
...
router.patch('/blooms-level/:id/status', isAuthenticated, isAdmin, async (req, res) => {
    try {
        const level = await BloomsLevel.findByPk(req.params.id);
        if (!level) return res.status(404).json({ success: false, message: 'Bloom\'s level not found' });
...
router.get('/difficulty-level', isAuthenticated, isAdmin, async (req, res) => {
    try {
...
router.post('/difficulty-level', isAuthenticated, isAdmin, async (req, res) => {
    try {
...
router.put('/difficulty-level/:id', isAuthenticated, isAdmin, async (req, res) => {
    try {
...
router.delete('/difficulty-level/:id', isAuthenticated, isAdmin, async (req, res) => {
    try {
        const level = await DifficultyLevel.findByPk(req.params.id);
        if (!level) return res.status(404).json({ success: false, message: 'Difficulty level not found' });
...
router.patch('/difficulty-level/:id/status', isAuthenticated, isAdmin, async (req, res) => {
    try {
        const level = await DifficultyLevel.findByPk(req.params.id);
        if (!level) return res.status(404).json({ success: false, message: 'Difficulty level not found' });
...
// Get all Units
router.get('/units', isAuthenticated, isAdmin, async (req, res) => {
    try {
...
// Toggle Unit status
router.patch('/units/:id/status', isAuthenticated, isAdmin, async (req, res) => {
    try {
        const unit = await Unit.findByPk(req.params.id);
        if (!unit) return res.status(404).json({ success: false, message: 'Unit not found' });
