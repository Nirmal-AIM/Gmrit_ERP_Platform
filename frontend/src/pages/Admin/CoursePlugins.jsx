...
const CoursePlugins = () => {
    ...
    const fetchAllData = async () => {
        try {
            ...
            const [bloomsRes, diffRes, unitsRes] = await Promise.all([
                api.get('/api/admin/course-plugins/blooms-level'),
                api.get('/api/admin/course-plugins/difficulty-level'),
                api.get('/api/admin/course-plugins/units')
            ]);
    ...
    const getEndpoint = () => {
        if (activeTab === 'blooms') return '/api/admin/course-plugins/blooms-level';
        if (activeTab === 'difficulty') return '/api/admin/course-plugins/difficulty-level';
        return '/api/admin/course-plugins/units';
    };
    ...
    const handleToggleStatus = async (id) => {
        try {
            await api.patch(`${getEndpoint()}/${id}/toggle-status`);
            fetchAllData();
        } catch (err) {
